unit synhighlighterdodolasmali;
...
type
  ...
  TSynDodolaSmaliSyn = class(TSynCustomHighlighter)
  private
    fRange: TRangeState;
    fLine: PChar;
...
const
  ...
  ShellScriptKeys: array[1..ShellScriptKeysCount] of string = (
    ...
    'bridge', 'varargs', 'declared', 'build', 'runtime', 'system', 'constructor', 'assert', 'break', 'case', 'const',
    'continue', 'default', 'do', 'else', 'extends', 'finally', 'for', 'goto', 'if', 'import', 'instanceof', 'new',
    'package', 'return', 'this', 'throw', 'throws', 'try', 'void', 'while');

  ShellScriptSecondKeysCount = 48;
  ShellScriptSecondKeys: array[1..ShellScriptSecondKeysCount] of string = (
    'init', 'clinit', 'wide', 'string', 'check', 'cast', 'instance',
    'nop', 'move', 'result', 'object', 'exception', 'array', 'length', 'neg', 'not',
    'invoke', 'direct', 'virtual', 'iget', 'iput', 'monitor', 'aget', 'aput',
    'enter', 'exit', 'mul', 'add', 'sub', 'rsub', 'div', 'rem', 'and', 'or', 'xor',
    'shl', 'shr', 'ushr', 'addr', 'instance', 'of', 'sget', 'sput', 'cmp', 'cmpl', 'cmpg',
    'fill', 'filled'
    );
  ShellScriptThirdKeyCount = 21;
  ShellScriptThirdKeys: array[1..ShellScriptThirdKeyCount] of string = (
    ...
    'nez', 'ltz', 'gez', 'gtz', 'lez', 'eqz', 'boolean'
  );
...
function TSynDodolaSmaliSyn.IsKeyword(const AKeyword: string): boolean;
begin
  Result :=  (fKeyWords.IndexOf(AKeyword) <> -1);
end; { IsKeyWord }

function TSynDodolaSmaliSyn.IsSecondKeyWord(aToken: string): Boolean;
begin
  Result := (fSecondKeys.IndexOf(aToken) <> -1);
end; { IsSecondKeyWord }

function TSynDodolaSmaliSyn.IsThirdKeyWord(aToken: string): Boolean;
begin
  Result := (FThirdKeyWords.IndexOf(aToken) <> -1);
end;

procedure TSynDodolaSmaliSyn.MakeMethodTables;
var
  I: Char;
begin
  for I := #0 to #255 do
...
constructor TSynDodolaSmaliSyn.Create(AOwner: TComponent);
var
  i: integer;
begin
...
procedure TSynDodolaSmaliSyn.SetLine(
...
destructor TSynDodolaSmaliSyn.Destroy;
begin
  fKeyWords.Free;
  fSecondKeys.Free;
  FThirdKeyWords.Free;
...
procedure TSynDodolaSmaliSyn.AnsiProc;
begin
  fTokenID := tkComment;
  case FLine[Run] of
    #0:
...
procedure TSynDodolaSmaliSyn.PasStyleProc;
begin
  fTokenID := tkComment;
  case FLine[Run] of
    #0:
...
procedure TSynDodolaSmaliSyn.CStyleProc;
begin
  case fLine[Run] of
    #0: NullProc;
    #10: LFProc;
...
procedure TSynDodolaSmaliSyn.DollarProc;
var
  cc: Char;
begin
  inc(Run);
...
procedure TSynDodolaSmaliSyn.DotProc;
...
procedure TSynDodolaSmaliSyn.BraceOpenProc;
begin
  inc(Run);
  fTokenID := tkSymbol;
end;

procedure TSynDodolaSmaliSyn.PointCommaProc;
begin
  inc(Run);
  fTokenID := tkSymbol;
end;

procedure TSynDodolaSmaliSyn.CRProc;
begin
  fTokenID := tkSpace;
  case FLine[Run + 1] of
    #10: inc(Run, 2);
...
procedure TSynDodolaSmaliSyn.IdentProc;
begin
...
procedure TSynDodolaSmaliSyn.LFProc;
begin
  fTokenID := tkSpace;
  inc(Run);
end;

procedure TSynDodolaSmaliSyn.NullProc;
begin
  fTokenID := tkNull;
end;

procedure TSynDodolaSmaliSyn.NumberProc;
begin
  inc(Run);
  fTokenID := tkNumber;
...
procedure TSynDodolaSmaliSyn.RoundOpenProc;
begin
  inc(Run);
  fTokenId := tkSymbol;
end;

procedure TSynDodolaSmaliSyn.SetThirdKeyWords(Value: TStrings);
var
  i: Integer;
begin
...
procedure TSynDodolaSmaliSyn.SlashProc;
begin
...
procedure TSynDodolaSmaliSyn.SpaceProc;
begin
  inc(Run);
  fTokenID := tkSpace;
...
procedure TSynDodolaSmaliSyn.StringProc;
var
  QuoteChar: Char;
begin
...
procedure TSynDodolaSmaliSyn.UnknownProc;
begin
  inc(Run);
...
procedure TSynDodolaSmaliSyn.Next;
begin
  fTokenPos := Run;
  case fRange of
    rsAnsi: AnsiProc;
...
function TSynDodolaSmaliSyn.GetDefaultAttribute(Index: integer): TSynHighlighterAttributes;
begin
  case Index of
...
function TSynDodolaSmaliSyn.GetEol: Boolean;
begin
  Result := False;
  if fTokenId = tkNull then Result := True;
end;

function TSynDodolaSmaliSyn.GetRange: Pointer;
begin
  Result := Pointer(PtrInt(fRange));
end;

function TSynDodolaSmaliSyn.GetToken: string;
var
  Len: LongInt;
begin
  Result := '';
...
procedure TSynDodolaSmaliSyn.GetTokenEx(out TokenStart: PChar;
...
function TSynDodolaSmaliSyn.GetTokenID: TtkTokenKind;
begin
  Result := fTokenId;
end;

function TSynDodolaSmaliSyn.GetTokenAttribute: TSynHighlighterAttributes;
begin
  case fTokenID of
...
function TSynDodolaSmaliSyn.GetTokenKind: integer;
begin
  Result := Ord(fTokenId);
end;

function TSynDodolaSmaliSyn.GetTokenPos: Integer;
begin
  Result := fTokenPos;
end;

procedure TSynDodolaSmaliSyn.ResetRange;
begin
  fRange := rsUnknown;
end;

procedure TSynDodolaSmaliSyn.SetRange(Value: Pointer);
begin
  fRange := TRangeState(PtrUInt(Value));
end;

procedure TSynDodolaSmaliSyn.SetSecondKeys(const Value: TStrings);
begin
  fSecondKeys.Assign(Value);
  DefHighLightChange(nil);
end;

function TSynDodolaSmaliSyn.GetIdentChars: TSynIdentChars;
begin
...
class function TSynDodolaSmaliSyn.GetLanguageName: string;
begin
  Result := LangName;
end;
...
initialization
  MakeIdentTable;
  RegisterPlaceableHighlighter(TSynDodolaSmaliSyn);
